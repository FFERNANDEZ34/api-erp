...
export class ProductController {
  ...
  async create(req: AuthenticatedRequest, res: Response) {
    const productSchema = z.object({
      ...
      taxTypeParamId: z.number().int("Tipo de afectación tributaria inválida."),
      unitMeasureParamId: z.number().int("Unidad de medida inválida."),
      purchasePrice: z.number().positive("El precio de compra debe ser mayor a cero."),
      salesPrice: z.number().positive("El precio de venta debe ser mayor a cero."),
    ...
    if (!subscriptionId || isNaN(companyId)) {
      return res.status(401).json({
        status: "fail",
        message: "Seguridad Multi-Tenant: Contexto de Holding o Empresa comercial no válido."
      });
    }
    ...
    return res.status(201).json({
      status: "success",
      message: "Artículo indexado correctamente en el inventario comercial de la empresa.",
      data: newProduct
    });
  }
  ...
  async update(req: AuthenticatedRequest, res: Response) {
    ...
    if (!subscriptionId || isNaN(productId)) {
      return res.status(401).json({ status: "fail", message: "Identificadores de transacción inválidos." });
    }
    ...
    const updatedProduct = await this.updateProductUseCase.execute(productId, subscriptionId, body);
  ...
  async getPaginated(req: AuthenticatedRequest, res: Response) {
    ...
    if (!subscriptionId || isNaN(companyId)) {
      return res.status(401).json({
        status: "fail",
        message: "No se proporcionó un contexto comercial válido para segmentar las existencias."
      });
    }
  ...
  async delete(req: AuthenticatedRequest, res: Response) {
    ...
    if (!subscriptionId || isNaN(productId)) {
      return res.status(401).json({ status: "fail", message: "Parámetros de baja inválidos." });
    }
    ...
    return res.status(200).json({
      status: "success",
      message: "El artículo ha sido dado de baja lógicamente del catálogo comercial."
    });
  }
}