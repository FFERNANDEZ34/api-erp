  ...
  async execute(data: CreateInvoiceInput) {
    ...
    try {
      ...
      const seriesRow = await DocumentSeriesModel.findOne({
        where: { id: data.seriesId, subscriptionId: data.subscriptionId, companyId: data.companyId },
        transaction,
        lock: transaction.LOCK.UPDATE // Evita que otro cajero lea este mismo correlativo en paralelo
      });

      if (!seriesRow || !seriesRow.isActive) {
        throw new Error('El talonario de series seleccionado no es válido o fue dado de baja.');
      }
      ...
      for (const item of data.details) {
        ...
        if (item.taxTypeCode === '10') { // Operación Gravada Comercial Estándar (18%)
          taxPercentage = 18.00;
        ...
        if (!productRow) {
          throw new Error(`El producto [${item.productName}] no existe en el catálogo general del holding.`);
        }
        ...
        if (item.unitMeasureCode === 'NIU' && currentStock < qty) {
          throw new Error(`Stock insuficiente para [${productRow.get('name')}]. Saldo en almacén: ${currentStock}, Solicitado: ${qty}`);
        }
        ...
        await productRow.update({ stock: calculatedFinalStock }, { transaction });
