...
export class UserController {
  ...
  async subscribe(req: Request, res: Response) {
    const subscribeSchema = z.object({
      ...
      companyName: z.string().min(2, "El nombre de su primera compañía es obligatorio"),
      companyRuc: z.string().length(11, "El RUC de la compañía debe tener exactamente 11 dígitos"),
  ...
  async login(req: Request, res: Response) {
    ...
    const tokens = await this.loginUserUseCase.execute(body.email, body.password);
  ...
  async logout(req: AuthenticatedRequest, res: Response) {
    ...
    if (!userId || !accessToken) {
      return res.status(401).json({ status: 'fail', message: 'Token no autorizado' });
    }

    await this.logoutUserUseCase.execute(userId, accessToken);
    return res.status(200).json({ message: 'Sesión cerrada con éxito. Token revocado.' });
  }
  ...
  async registerSubUser(req: AuthenticatedRequest, res: Response) {
    ...
    return res.status(201).json({
      status: 'success',
      message: 'Colaborador registrado y asignado a sus respectivas compañías y locales con éxito.',
      data: newUser
    });
  }
}